...
def parse_args(training=False):
    ...
    # ======================================================
        ...
        parser.add_argument("--start-from-scratch", default=None, help="start training from scratch")
...
def read_config(config_path):
    """
    Read config file. If file ends with .py, use mmengine.Config.
    Otherwise, use OmegaConf (which supports variable interpolation like ${var}).
    
    Args:
        config_path: Path to config file
        
    Returns:
        Config object (mmengine.Config)
    """
    config_path = str(config_path)
    
    # If .py file, use mmengine.Config
    if config_path.endswith('.py'):
        cfg = Config.fromfile(config_path)
        return cfg
    
    # Load with OmegaConf (supports ${var} interpolation)
    omega_cfg = OmegaConf.load(config_path)
    
    # Convert OmegaConf DictConfig to regular dict, then to mmengine.Config
...
def parse_configs(training=False):
    ...
    cfg_args = merge_args(cfg, args, training)
    # import pdb; pdb.set_trace()
    return cfg_args
