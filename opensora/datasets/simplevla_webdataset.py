...
class SimpleVLARLinfDataset(torch.utils.data.IterableDataset):
    def __init__(
        self,
        shards_pattern: str,
        stats_path: str,
        *,
        ...
        print(f"[SimpleVLA-JZN] 从 pattern '{shards_pattern}' 找到 {len(shards)} 个 shard")
    ...
    def _split_to_windows(
        ...
        for v_bytes, a_bytes, m_bytes in src:
            ...
            finish_step = int(meta.get("finish_step")) + self.finish_step_shift - 1
            for start in range(0, finish_step - self.Ta + 1, self.stride):
                ...
                video = torch.from_numpy(vid_win).float().clone() / 127.5 - 1
                # video = torch.from_numpy(vid_win).float().clone() / 127.5 - 1
                # video = torch.tensor(vid_win).float() / 127.5 - 1
                video = video.permute(3, 0, 1, 2)

                video_len = self.Ta + self.To

                _, real_video_len, _, _ = video.shape

                assert video_len==real_video_len, "video len is not right"

                # act_np = action_np[start : start + self.Ta]
                # jzn modified ： o_{t-To:t-1} and a_{t:t+Ta}, get  o_{t:t+Ta}  {}是闭区间
                # python index 原因，+ 1
                act_np = action_np[start + 1 : start + self.Ta + 1]
                ...
                # print(f"video shape is {video.shape}")
