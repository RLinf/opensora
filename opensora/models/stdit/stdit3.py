...
class ActionEncoder(nn.Module):
    def __init__(self, hidden_size, action_dim, cfg=False, training=False, pad_action_num = 1):
        """
        初始化ActionEncoder
        :param embedding_dim: 每个整数的嵌入维度
        :param output_dim: 最终输出的向量表征维度
        """
        super(ActionEncoder, self).__init__()
        self.action_dim = action_dim
        self.fc = nn.Sequential(
            nn.Linear(self.action_dim, hidden_size*4),
            nn.SiLU(),
        ...
        self.none_action = nn.Embedding(1, hidden_size)
        self.pad_action_num = pad_action_num
    
    def forward(self, x):
        batch_size, action_num, action_dim  = x.size()
        x = x.reshape(-1, action_dim)
        output = self.fc(x)

        output = output.reshape(batch_size, action_num, self.hidden_size)
        none_action = self.none_action.weight  # 获取权重，形状是 (1, hidden_size)
        output = torch.cat((none_action.repeat(batch_size, self.pad_action_num, 1), output), dim=1)

        if self.cfg and self.training:
            expanded_none_action = none_action.repeat(batch_size, action_dim+1, 1) # 形状变为 (batch_size, hidden_size)
            mask = [random.random() < 0.1 for _ in range(batch_size)]
            mask = torch.tensor(mask).to(x.device)  # 转换为Tensor并保证在相同的设备上
            output = torch.where(mask[:, None, None], expanded_none_action, output) # unsqueeze(1)使得mask变为 (batch_size, 1)
...
class STDiT3Block(nn.Module):
    ...
    def forward(
        ...
        B, T, S, C = x.shape
        if self.temporal:
            t = t.reshape(B, 1, 6, -1) + self.scale_shift_table[None]
            t = rearrange(t, "B 1 n h -> B 1 1 (n h)")
        else:
            t = t.reshape(B, T, 6, -1) + self.scale_shift_table[None]
            t = rearrange(t, "B T n h -> B T 1 (n h)")
        shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = t.chunk(6, dim=-1)
        t0 = t0.reshape(B, 1, 6, -1)
        if x_mask is not None:
            shift_msa_zero, scale_msa_zero, gate_msa_zero, shift_mlp_zero, scale_mlp_zero, gate_mlp_zero = (
                self.scale_shift_table[None] + t0
            ).chunk(6, dim=2)
        ...
        if x_mask is not None:
            ...
            x_m = torch.where(x_mask[:, :, None, None], x_m, x_m_zero)
        ...
        if x_mask is not None:
            ...
            x_m_s = torch.where(x_mask[:, :, None, None], x_m_s, x_m_s_zero)
...
class STDiT3(PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.action_embedder = ActionEncoder(config.hidden_size, config.action_dim, config.cfg, config.training, config.pad_action_num)
    ...
    def forward(self, x, timestep, y, mask=None, x_mask=None, fps=None, height=None, width=None, **kwargs):
        ...
        action_embedding = self.action_embedder(y)  # action_embedding: [12, 1152], y.shape: [1, 8, 7]
        ...
        t_s = repeat(t, 'b d -> (b c) d', c=T)  # t_s: [12, 1152]

        t_s = t_s + action_embedding
        t_s_mlp = self.t_block(t_s)
        t_t_mlp = self.t_block(t)
        t0 = t0_mlp = None
        if x_mask is not None:
            t0_timestep = torch.ones_like(timestep) * 15
        ...
        for spatial_block, temporal_block in zip(self.spatial_blocks, self.temporal_blocks):
            x = auto_grad_checkpoint(spatial_block, x, t_s_mlp, x_mask, t0_mlp, T, S)
            x = auto_grad_checkpoint(temporal_block, x, t_t_mlp, x_mask, t0_mlp, T, S)
